...
export default function LoyaltyPanel({
  ...
  const progress = nextFloor === currentLevelFloor
    ? 0
    : Math.min(100, ((xp - currentLevelFloor) / (nextFloor - currentLevelFloor)) * 100);
    ...
    <section className="panel loyalty-panel">
      <div className="panel-head">
        <div>
          ...
          <h2>Level {level}</h2>
          <p>Lifetime XP never decreases. Supply Credits may be redeemed for bonus rewards.</p>
        </div>
        <Link className="button secondary" href="/rewards">View rewards</Link>
      </div>
      ...
      <div className="loyalty-metrics">
        <article>
          ...
          <span>{Math.max(0, nextFloor - xp).toLocaleString()} XP to Level {level + 1}</span>
        </article>
        <article>
          <small>SUPPLY CREDITS</small>
          <strong>{credits}</strong>
          <span>Estimated reward contribution: ${creditValue.toFixed(2)}</span>
        </article>
        <article>
          <small>LOYALTY MULTIPLIER</small>
          <strong>{Number(loyalty?.xp_multiplier ?? 1).toFixed(2)}×</strong>
          <span>Caps at 1.10× after 36 paid months</span>
        </article>
        <article>
          <small>PAID STREAK</small>
          <strong>{loyalty?.consecutive_paid_months ?? 0} months</strong>
          <span>Credits are not multiplied</span>
        </article>
      </div>
